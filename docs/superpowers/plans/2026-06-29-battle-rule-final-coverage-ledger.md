...
截至天气、场地和场上状态边界 fixture 接入批次，当前覆盖报表中已有 `87` 个登记项为 `IMPLEMENTED`。其中部分登记项已经覆盖多个规则行为，例如场地持续时间、撒场、技能状态效果和特性标签增伤。折算到本账本后，V1 最终目标先定为 `312` 条规则行为，当前已覆盖 `240` 条，剩余 `72` 条。
...
| 技能效果行为族 | 39 | 13 | 26 | 能力阶级、主要状态、HP 吸取/反伤/回复、强制替换、复制、封锁、清除、交换和失败条件。 |
| 特性效果行为族 | 36 | 22 | 14 | 入场、攻击前、防守前、命中后、天气/场地联动、属性吸收、状态免疫、规则绕过和伤害修正。 |
| 道具效果行为族 | 18 | 8 | 10 | 消耗、回复、状态解除、伤害增减、持续时间延长、一次性免死、锁招和抗性减伤。 |
| 随机、回放和对照测试基础 | 4 | 4 | 0 | 固定随机序列、事件流稳定、回放复算和对照测试结果归档。 |
| **合计** | **312** | **240** | **72** | 当前“已覆盖”按行为账本粗映射；覆盖报表实际登记项为 `87/87`。 |
...
由于部分报表项已经覆盖多个行为，例如场地持续时间、撒场、技能状态效果或特性标签增伤，映射到最终账本时会折算为多个行为；所以上方总量拆分里的“当前已覆盖”是 `240`，而不是报表登记项 `87`。
...
2. 继续补技能效果行为族：状态复制、封锁技能、解除场地或一侧状态、道具移除和特性变更。
3. 补特性和道具剩余行为族：击中要害增伤、低威力技能增伤、道具一次性消耗触发、锁招道具的行动选择限制。
4. 收尾格式和生命周期缺口：选择阶段、自定义格式约束、濒死后补位细节和战斗结束事件边界。
