...
class BattleRuleCoverageService(
		...
		listOf(
			...
			item(
				...
				fixtures = listOf(
					...
					"golden-replay-pins-random-trace-event-fragment-and-final-hp",
				),
				references = listOf(
				...
				note = "已覆盖固定随机脚本拒绝额外消费、随机 trace 保存上界/原因/值、严格 replay 复算事件片段和最终状态、随机 trace 被篡改时立即失败，以及黄金 replay 固定随机 trace、事件片段和最终 HP。",
			),
			item(
	...
	private fun item(
		...
		BattleRuleCoverageItemResponse(
			...
			fixtures = fixtures.map { staticFixture(it) },
			referenceUrls = references,
			note = note,
		)

	private fun staticFixture(code: String): BattleRuleCoverageFixtureResponse =
