 ...
 * 战斗规则覆盖报告的静态目录。
 *
 * 这里仅维护“规则项、公开来源和 fixture 名称”的人工清单，不读取数据库，也不关心最新运行结果。
 * 运行时 fixture 匹配和完整性校验留在 [BattleRuleCoverageService]，避免静态目录和 Spring/Jimmer 依赖混在一起。
 */
internal object BattleRuleCoverageCatalog {
	fun coverageItems(): List<BattleRuleCoverageItemResponse> =
		listOf(
			item(
