 ...
 * MyBatis-Plus 基础配置
 *
 * <p>为项目统一配置 MyBatis-Plus 的基础能力，包括分页插件、乐观锁等常用拦截器 并通过程序化方式注册 Mapper 接口扫描，支持按模块结构组织 mapper 包
 *
 * <p>注意：在模块化多 module 项目中，使用通配包路径（如 io.github.lishangbu.avalon.**.mapper）可以方便统一扫描 但部分 IDE
 * 对此类通配路径的静态检查可能会报“包不存在”的提示，这属于 IDE 检查问题， 运行时 Spring Boot 与 MyBatis 会正确加载 mapper 接口；如需在 IDE
 * 中消除警告可以添加相应的忽略注释。
 *
 * @author avalon-team
 * @since 2025/12/11
