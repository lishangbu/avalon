 ...
 *
 * <p>转换规则参照 `BerryFlavorMapstruct`： - id -> Long - internalName <- name - name 使用本地化名称（从 names
 * 列表中解析）
 */
@Mapper(componentModel = SPRING)
public abstract class BerryFirmnessMapstruct extends AbstractMapstruct {

  @Mapping(target = "id", expression = "java(pokeApiBerryFirmness.id().longValue())")
  @Mapping(target = "internalName", source = "name")
  @Mapping(
    target = "name",
    expression = "java(resolveLocalizedNameFromNames(pokeApiBerryFirmness.names(), pokeApiBerryFirmness.name()))")
