 ...
 * 验证现代主系列随机相邻对手目标。
 *
 * 场景类型：随机目标候选收集、随机 trace、无候选目标取消 场景。
...
class BattleRandomTargetPublicReferenceTests {
	...
	fun `random adjacent opponent target chooses scripted capable opponent`() {
		...
		scenario.assertNamed("random-adjacent-opponent-target-chooses-scripted-capable-opponent")
	...
	fun `random adjacent opponent with no capable opponent cancels before pp and random`() {
		...
		scenario.assertNamed("random-adjacent-opponent-with-no-capable-opponent-cancels-before-pp-and-random")
