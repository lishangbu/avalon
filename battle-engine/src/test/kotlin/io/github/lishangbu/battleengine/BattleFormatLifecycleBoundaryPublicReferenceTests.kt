 ...
 *
 * 场景类型：格式入口不变量、队伍合法性聚合、行动提交结构化错误、濒死胜负事件顺序和离场状态清理 fixture。
 ...
 * 以稳定事件顺序暴露给 replay、管理端和后续公开 fixture。
 *
...
class BattleFormatLifecycleBoundaryPublicReferenceTests {
	...
	fun `format rejects blank code`() {
		val fixture = fixture(
			...
			expectedSummary = "格式入口立即拒绝空白 code，避免后续 fixture 和 replay 失去稳定格式标识。",
		)
		...
		fixture.assertNamed("format-rejects-blank-code")
	...
	fun `format rejects default level below one`() {
		...
		fixture.assertNamed("format-rejects-default-level-below-one")
	...
	fun `format rejects zero max turns`() {
		...
		fixture.assertNamed("format-rejects-zero-max-turns")
	...
	fun `initial state rejects duplicate side ids`() {
		...
		fixture.assertNamed("initial-state-rejects-duplicate-side-ids")
	...
	fun `initial state rejects active count mismatch`() {
		...
		fixture.assertNamed("initial-state-rejects-active-count-mismatch")
	...
	fun `preparation validator collects direct restrictions on one participant`() {
		...
		fixture.assertNamed("preparation-validator-collects-direct-restrictions-on-one-participant")
		assertEquals(
	...
	fun `preparation validator records duplicate item resource ids`() {
		...
		fixture.assertNamed("preparation-validator-records-duplicate-item-resource-ids")
	...
	fun `preparation validator ignores duplicate clauses when disabled`() {
		...
		fixture.assertNamed("preparation-validator-ignores-duplicate-clauses-when-disabled")
	...
	fun `action validator reports missing actor`() {
		...
		fixture.assertNamed("action-validator-reports-missing-actor")
	...
	fun `action validator reports bench actor is not active`() {
		...
		fixture.assertNamed("action-validator-reports-bench-actor-is-not-active")
	...
	fun `action validator reports fainted active skill user`() {
		...
		fixture.assertNamed("action-validator-reports-fainted-active-skill-user")
	...
	fun `action validator reports target is not active`() {
		...
		fixture.assertNamed("action-validator-reports-target-is-not-active")
	...
	fun `action validator reports fainted active target`() {
		...
		fixture.assertNamed("action-validator-reports-fainted-active-target")
	...
	fun `action validator reports switch target already active`() {
		...
		fixture.assertNamed("action-validator-reports-switch-target-already-active")
	...
	fun `battle ended validation keeps specific action violations`() {
		...
		fixture.assertNamed("battle-ended-validation-keeps-specific-action-violations")
	...
	fun `participant faint event precedes battle ended event`() {
		...
		fixture.assertNamed("participant-faint-event-precedes-battle-ended-event")
	...
	fun `battle ending turn does not append normal turn ended`() {
		...
		fixture.assertNamed("battle-ending-turn-does-not-append-normal-turn-ended")
	...
	fun `simultaneous last participants faint produces draw result`() {
		...
		fixture.assertNamed("simultaneous-last-participants-faint-produces-draw-result")
	...
	fun `switch out clears disable taunt torment and heal block`() {
		...
		fixture.assertNamed("switch-out-clears-disable-taunt-torment-and-heal-block")
	...
	fun `forced switch clears binding locked choice and substitute state`() {
		...
		fixture.assertNamed("forced-switch-clears-binding-locked-choice-and-substitute-state")
