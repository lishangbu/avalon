...
/**
 * `BattleParticipant` 的运行态操作集合。
 *
 * `BattleParticipant` 作为不可变快照只负责保存字段和构造期不变量；这些扩展函数负责把一次战斗规则结算
 * 转换成新的快照。拆到独立文件后，模型类本身不会继续承载所有状态迁移细节，调用点仍然使用
 * `participant.receiveDamage(...)` 这样的领域语言，避免为了重构引入额外服务对象或兼容层。
 *
 * 这里的函数都不直接产生战斗事件，只返回更新后的成员快照。事件记录由更上层的回合流程负责追加，
 * 这样同一个状态变更可以在命中、替身、异常状态、道具触发等不同规则路径中复用，而不会把 UI/replay
 * 可观察事实混进底层数据结构。
 */
/**
 * 判断成员是否仍可继续战斗。
 ...
 * 畏缩、混乱、回复封锁、挑衅、定身法、无理取闹和束缚属于临时状态，离场时会被清除。后续接入锁招、寄生等
 * 离场即消失的状态时，也应在这里统一清理。
 */
