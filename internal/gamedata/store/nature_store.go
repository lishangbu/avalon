...
type natureTransactionStore struct {
	parent   *Store
...
func (w *natureTransactionStore) Create(ctx context.Context, record nature.CreateRecord) (nature.Nature, error) {
...
func (w *natureTransactionStore) Update(ctx context.Context, record nature.UpdateRecord) (nature.Nature, error) {
	digest, err := idempotency.Digest(record.Nature)
	if err != nil {
...
// NewNatureStore 创建 Nature 应用服务使用的窄存储适配器。
func NewNatureStore(store *Store) *NatureStore { return &NatureStore{Store: store} }

// Get 查询指定 Nature。
func (s *NatureStore) Get(ctx context.Context, id snowflake.ID) (nature.Nature, error) {
...
// List 返回 Nature 资料页。
func (s *NatureStore) List(ctx context.Context, query nature.ListQuery) (nature.Page, error) {
