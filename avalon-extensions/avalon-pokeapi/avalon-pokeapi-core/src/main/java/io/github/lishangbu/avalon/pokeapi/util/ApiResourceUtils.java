...
/// 资源工具类
///
/// 提供从 ApiResource 的 URL 中提取 ID 的辅助方法
///
/// @author lishangbu
/// @since 2025/8/9
@Slf4j
public abstract class ApiResourceUtils {
    ...
    public static Integer getId(APIResource apiResource) {
        if (apiResource == null) {
            log.warn("ApiResource is null, cannot extract ID.");
            return null;
        }
        Matcher matcher = PATTERN.matcher(apiResource.url());
        ...
        if (matcher.find()) {
            ...
            log.warn("没有找到匹配的数字,URL不匹配预期格式: [{}]", apiResource.url());
            return null;
        }
    }
}
