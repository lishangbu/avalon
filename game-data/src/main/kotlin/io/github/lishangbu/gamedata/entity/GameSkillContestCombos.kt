 ...
 * 技能评价组合持久化实体。
 *
 * 对应 `game_skill_contest_combo` 表，仅承载一条技能评价组合记录及其标量属性。
 * 关联项保留为独立 ID，避免把其他功能的实体聚合进当前资料对象；新增记录的主键统一由 CosId 生成。
 */
@Entity
@Table(name = "game_skill_contest_combo")
interface GameSkillContestCombos {
	/**
	 * 技能评价组合记录的唯一标识。
	 *
	 ...
	 * 技能评价组合的组合类型。
	 *
	 * 对应数据库 `combo_type` 列，写入时必须提供，最大长度为 40 个字符。
	 */
	 ...
	 * 技能评价组合引用的关联技能 ID。
	 *
	 * 对应数据库 `related_skill_id` 列，写入时必须提供；关联标识以 Long 标量保存，不在该实体中聚合关联对象。
	 */
