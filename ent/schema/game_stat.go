...
// GameStat 定义 game_stat 表的持久化结构。
type GameStat struct {
	ent.Schema
}

// Fields 返回 game_stat 表全部字段及其数据库约束。
func (GameStat) Fields() []ent.Field {
	return []ent.Field{
...
// Annotations 固定 game_stat 的表名、注释、复合主键和检查约束。
func (GameStat) Annotations() []schema.Annotation {
	return []schema.Annotation{
		schema.Comment("实时游戏资料：战斗属性。"),
		entsql.WithComments(true),
		entsql.Annotation{Table: "game_stat", Checks: map[string]string{
			"game_stat_code_check":    "code::text ~ '^[a-z][a-z0-9-]{1,63}$'::text",
			"game_stat_name_check":    "char_length(name::text) >= 1 AND char_length(name::text) <= 80 AND name::text = btrim(name::text)",
			"game_stat_version_check": "version > 0",
		}},
	}
}
