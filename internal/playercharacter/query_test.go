...
func TestQueryServiceOnlyExposesPublicCharacterAfterCallerHasActiveBinding(t *testing.T) {
	...
	service := playercharacter.NewQueryService(adapters, adapters, presence, func() time.Time { return now })
	...
	if adapters.displayNameKey != "星界旅人" {
		t.Fatalf("displayNameKey = %q", adapters.displayNameKey)
	}
}

type adaptersStub struct {
	active         playercharacter.ActiveBinding
	found          playercharacter.PlayerCharacter
	displayNameKey string
}

func (s *adaptersStub) GetOwned(context.Context, snowflake.ID, snowflake.ID) (playercharacter.PlayerCharacter, error) {
	return playercharacter.PlayerCharacter{}, nil
}

func (s *adaptersStub) ListOwned(context.Context, snowflake.ID, bool) ([]playercharacter.PlayerCharacter, error) {
	return nil, nil
}

func (s *adaptersStub) GetActive(context.Context, snowflake.ID) (playercharacter.ActiveBinding, error) {
	return s.active, nil
}

func (s *adaptersStub) FindActiveByDisplayNameKey(_ context.Context, key string) (playercharacter.PlayerCharacter, error) {
