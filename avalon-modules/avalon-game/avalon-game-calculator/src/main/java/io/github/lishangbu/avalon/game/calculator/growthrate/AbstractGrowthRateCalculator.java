...
/// - 实际的成长值计算由子类在 [#tryCalculateGrowthRate(int)] 中实现
///
/// 设计要点：
/// - 名称匹配使用不区分大小写的比较，调用方可通过 [#support(String)] 快速判断是否支持某种成长速率
...
public abstract class AbstractGrowthRateCalculator implements GrowthRateCalculator {
    ...
    /// 计算指定等级的成长值
    ///
    /// 本实现负责边界处理：当 `level <= 0` 返回 `0`；当 `level == 1` 返回`0`；
    /// 其余情况委托给 [#tryCalculateGrowthRate(int)] 由子类完成具体计算
    ///
