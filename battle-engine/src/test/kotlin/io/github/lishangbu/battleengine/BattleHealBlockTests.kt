...
class BattleHealBlockTests {
	...
	fun `status skill applies heal block to target for five turns`() {
		...
		scenario.assertNamed("status-skill-applies-heal-block-to-target")
	...
	fun `heal blocked participant cannot use self healing status skill`() {
		...
		scenario.assertNamed("heal-blocked-participant-cannot-use-self-healing-status-skill")
	...
	fun `heal blocked participant cannot use draining damage skill`() {
		...
		scenario.assertNamed("heal-blocked-participant-cannot-use-draining-damage-skill")
	...
	fun `heal block suppresses end turn held item healing`() {
		...
		scenario.assertNamed("heal-block-suppresses-end-turn-held-item-healing")
	...
	fun `heal block clears when end turn duration reaches zero`() {
		...
		scenario.assertNamed("heal-block-clears-when-end-turn-duration-reaches-zero")
