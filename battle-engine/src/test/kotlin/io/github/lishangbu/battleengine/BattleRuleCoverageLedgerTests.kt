 ...
 * 才需要新增行为测试并调整对应规则族的 `ruleCount` 或代表测试类。
 */
class BattleRuleCoverageLedgerTests {
	@Test
	fun `modern battle rule ledger keeps all covered behavior groups documented`() {
		...
		coverageGroups.forEach { group ->
			...
			assertTrue(group.representativeTestClasses.isNotEmpty(), "${group.code} should reference behavior tests")
			group.representativeTestClasses.forEach { testClassName ->
	...
	private data class CoverageGroup(
		...
		val representativeTestClasses: List<String>,
	)
	...
	private companion object {
		private val coverageGroups = listOf(
			CoverageGroup(
				...
				representativeTestClasses = listOf(
