 ...
 * 公开规则对照测试的元数据。
 *
 * 每条战斗规则 fixture 都需要说明名称、公开来源、输入摘要和期望摘要。这个类型只服务测试层，
 * 不参与生产代码，也不替代具体断言；它的职责是让后续规则实现都能留下可审计的外部对照依据。
 */
internal data class PublicBattleRuleFixture(
	...
	init {
		...
		require(sourceUrls.isNotEmpty()) { "fixture sourceUrls must not be empty" }
		require(sourceUrls.all { it.startsWith("https://") }) { "fixture sourceUrls must use https" }
