 ...
 * 种类精灵变种持久化实体。
 *
 * 对应 `game_species_creature_variety` 表，仅承载一条种类精灵变种记录及其标量属性。
 * 关联项保留为独立 ID，避免把其他功能的实体聚合进当前资料对象；新增记录的主键统一由 CosId 生成。
 */
@Entity
@Table(name = "game_species_creature_variety")
interface GameSpeciesCreatureVarieties {
	/**
	 * 种类精灵变种记录的唯一标识。
	 *
	 ...
	 * 用于标识种类精灵变种的“默认变种”状态。
	 *
	 * 对应数据库 `default_variety` 列，写入时必须提供。
	 */
