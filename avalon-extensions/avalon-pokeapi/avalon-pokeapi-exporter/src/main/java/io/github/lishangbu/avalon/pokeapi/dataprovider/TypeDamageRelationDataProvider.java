...
public class TypeDamageRelationDataProvider
  ...
  public List<TypeDamageRelationExcelDTO> fetch(
      PokeDataTypeEnum typeEnum, Class<TypeDamageRelationExcelDTO> typeDamageRelationExcelDTO) {
                ...
                namedApiResource ->
                    (Type)
                        pokeApiService.getEntityFromUri(
                            PokeDataTypeEnum.TYPE, NamedApiResourceUtils.getId(namedApiResource)))
            .toList();
    for (Type type : types) {
      TypeRelations typeRelations = type.damageRelations();
      addDamageFrom(typeDamageRelationCache, typeRelations.doubleDamageFrom(), type.id(), 2.0f);
    ...
    return typeDamageRelationCache.entrySet().stream()
        .map(
            entry -> {
              ...
              return new TypeDamageRelationExcelDTO(attackerId, defenderId, entry.getValue());
            })
        .sorted(
