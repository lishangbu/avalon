...
// abilityTransactionStore 闅旂鐗规€ц祫鏂?Writer 鐨勬柟娉曢泦鍚堬紝閬垮厤涓嶅悓璧勬枡绫诲瀷鐨勫懡浠ょ鍚嶄簰鐩歌€﹀悎銆?
type abilityTransactionStore struct {
	parent   *Store
...
func (w *abilityTransactionStore) Create(ctx context.Context, record ability.CreateRecord) (ability.Ability, error) {
...
func (s *Store) GetAbility(ctx context.Context, abilityID snowflake.ID) (ability.Ability, error) {
...
func (s *Store) ListAbilities(ctx context.Context, query ability.ListQuery) (ability.Page, error) {
...
func (w *abilityTransactionStore) Update(ctx context.Context, record ability.UpdateRecord) (ability.Ability, error) {
	digest, err := idempotency.Digest(struct {
...
func (w *abilityTransactionStore) Disable(ctx context.Context, record ability.DisableRecord) error {
	digest, err := idempotency.Digest(struct {
		AbilityID       snowflake.ID
		ExpectedVersion int64
