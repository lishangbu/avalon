 ...
 * 验证现代主系列行动流程边界。
 *
 * 场景类型：保护、命中前短路、目标有效性和濒死后行动取消 场景。
...
class BattleActionFlowBoundaryTests {
	...
	fun `skill unaffected by protection damages protected target`() {
		...
		scenario.assertNamed("protection-bypass-skill-damages-protected-target")
	...
	fun `protection block happens before accuracy random`() {
		...
		scenario.assertNamed("protection-blocks-before-accuracy-random")
	...
	fun `protection bypass still performs accuracy check before damage`() {
		...
		scenario.assertNamed("protection-bypass-still-performs-accuracy-check")
		assertEquals(100, resolved.participant("protector")?.currentHp)
	...
	fun `fainted queued actor cannot execute later action`() {
		...
		scenario.assertNamed("fainted-queued-actor-cannot-execute-later-action")
	...
	fun `action with no battle capable selected target consumes no pp`() {
		...
		scenario.assertNamed("action-with-no-battle-capable-selected-target-consumes-no-pp")
