...
/**
 * 变化技能成功后的使用者 HP 效果结算器。
 *
 * 变化技能不会进入普通伤害公式，也没有“本次造成的实际伤害量”。因此它们的自我回复、天气变量回复和建立替身
 * 不应该塞进伤害后 HP 效果类里，否则读代码时会误以为这些效果也参与吸取/反作用/低体力道具/倒下判定那条顺序。
 *
...
internal class BattleStatusSkillHpEffects {
	 ...
	 *
	 * 效果按技能槽中的 [BattleSkillSlot.hpEffects] 顺序执行。固定回复和天气变量回复最终汇入同一个最大 HP 比例
	 * helper；替身单独处理 HP 支付和替身 HP 写入。其它 HP 效果属于伤害技能后效，在这里直接跳过。
	 */
	fun apply(
		state: BattleState,
		actorId: String,
	 ...
	 * 这里不消费随机数，也不判断技能是否应该失败；调用方已经确认变化技能成功。
	 */
