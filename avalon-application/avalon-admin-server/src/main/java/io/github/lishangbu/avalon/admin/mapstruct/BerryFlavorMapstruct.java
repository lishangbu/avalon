...
public abstract class BerryFlavorMapstruct extends AbstractMapstruct {

  @Mapping(target = "id", expression = "java(pokeApiBerryFlavor.id().longValue())")
  @Mapping(target = "internalName", source = "name")
  @Mapping(target = "name", expression = "java(resolveLocalizedNameFromNames(pokeApiBerryFlavor.names(), pokeApiBerryFlavor.name()))")
