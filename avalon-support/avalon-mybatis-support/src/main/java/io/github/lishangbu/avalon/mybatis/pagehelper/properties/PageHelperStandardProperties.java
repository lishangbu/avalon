 ...
 * 这样会导致配置文件里的kebab-case风格配置映射到配置类中还是kebab-case而不是camelCase， 之后设置属性的时候就会因为找不到camelCase的配置导致失效。 <br>
 * 所有kebab-case风格的配置项都会产生这个问题，即配置不生效。 <br>
 * 这个类不继承{@link java.util.Properties}，用于提前接收配置文件中的配置项， 由于没有继承{@link
 * java.util.Properties}，SpringBoot可以做正常的风格转换， 转换完成后再将camelCase风格的配置放到原来的{@link
 * PageHelperProperties}中
 *
 * @author showen
 */
...
public class PageHelperStandardProperties {
  ...
  public void setUseSqlserver2012(Boolean useSqlserver2012) {
    ...
    Optional.ofNullable(useSqlserver2012)
        .ifPresent(v -> properties.setProperty("useSqlserver2012", v.toString()));
  }
  ...
  public void setBoundSqlInterceptors(String boundSqlInterceptors) {
    ...
    Optional.ofNullable(boundSqlInterceptors)
        .ifPresent(v -> properties.setProperty("boundSqlInterceptors", v));
  }
  ...
  public void setKeepOrderBy(Boolean keepOrderBy) {
    ...
    Optional.ofNullable(keepOrderBy)
        .ifPresent(v -> properties.setProperty("keepOrderBy", v.toString()));
  }
  ...
  public void setKeepSubSelectOrderBy(Boolean keepSubSelectOrderBy) {
    ...
    Optional.ofNullable(keepSubSelectOrderBy)
        .ifPresent(v -> properties.setProperty("keepSubSelectOrderBy", v.toString()));
  }
  ...
  public void setAsyncCount(Boolean asyncCount) {
    ...
    Optional.ofNullable(asyncCount)
        .ifPresent(v -> properties.setProperty("asyncCount", v.toString()));
  }
  ...
  public void setOrderBySqlParser(String orderBySqlParser) {
    ...
    Optional.ofNullable(orderBySqlParser)
        .ifPresent(v -> properties.setProperty("orderBySqlParser", v));
  }
  ...
  public void setSqlServerSqlParser(String sqlServerSqlParser) {
    ...
    Optional.ofNullable(sqlServerSqlParser)
        .ifPresent(v -> properties.setProperty("sqlServerSqlParser", v));
  }
}
