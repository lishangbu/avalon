 ...
 * `requiresTargetPendingPriorityDamagingSkill` 表示技能只有在目标本回合仍未行动且正准备使用先制伤害技能时才能继续。
 * 快手还击用它拦截对手先制攻击；目标准备变化技能或普通优先度攻击时都会在命中前失败。
 ...
 * 判定读取行动排序阶段计算出的有效优先度，因此资料、特性或场地带来的先制提升会走同一条保护 gate。
