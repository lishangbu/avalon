...
![Sa-Token](https://img.shields.io/badge/Sa--Token-authentication-2F7D32)
...
Avalon 是面向后台管理和玩家对战场景的 Kotlin Spring Boot 后端服务，提供 Sa-Token 认证、RBAC、系统管理、定时任务和 S3 对象存储基础能力。管理端前端仓库请参考 [avalon-admin-ui](https://github.com/lishangbu/avalon-admin-ui)。

## 导航

[管理端仓库](https://github.com/lishangbu/avalon-admin-ui) | [S3 Starter](./s3-spring-boot-starter/README.md) | [领域语言](./CONTEXT.md)
...
| `security` | Sa-Token 登录、Jimmer token 状态持久化和 RBAC 运行时。 |
| `system` | 系统管理 API，覆盖用户、角色、权限和定时任务管理。 |
...
Sa-Token 默认通过 `avalon-token` 请求头传递登录凭证，超时和活跃超时由 `sa-token` 配置控制。
...
| --- | --- |
| 当前会话 | `/api/session` |
| 登录与注销 | `/api/auth/login`、`/api/auth/logout` |
...
登录成功后，客户端通过 `avalon-token` 请求头访问受保护 API；角色和权限始终从 Jimmer RBAC 关系实时读取。
...
| 改动范围 | 建议命令 |
| --- | --- |
| 登录态、RBAC、OpenAPI 或应用装配 | `./gradlew :app:test --tests '*SaTokenAuthenticationApiTests' --tests '*ApiAuthorizationContractTests' --tests '*OpenApiDocumentationTests'` |
...
- [领域语言](./CONTEXT.md)
- [当前资料快照决策](./docs/adr/0001-current-game-data-snapshot.md)
- [数据库基线策略](./docs/adr/0002-liquibase-baseline-policy.md)
- [管理端 OpenAPI 权威](./docs/adr/0003-admin-openapi-authority.md)
