...
class BattleRuntimeSnapshotServiceTests(
	...
	fun `skill slot assembly includes explicit battle rule effects`() {
		...
		assertThat(slots.getValue(252).usableOnlyFirstSkillActionSinceEntering).isTrue()
		assertThat(slots.getValue(252).makesContact).isTrue()
		assertThat(slots.getValue(660).usableOnlyFirstSkillActionSinceEntering).isTrue()
		assertThat(slots.getValue(660).makesContact).isTrue()
		assertThat(slots.getValue(660).priority).isEqualTo(2)
		assertThat(slots.getValue(389).requiresTargetPendingDamagingSkill).isTrue()
		assertThat(slots.getValue(389).makesContact).isTrue()
		assertThat(slots.getValue(918).requiresTargetPendingPriorityDamagingSkill).isTrue()
		assertThat(slots.getValue(918).makesContact).isTrue()
		assertThat(slots.getValue(918).volatileStatusApplications.single().status).isEqualTo(BattleVolatileStatus.FLINCH)
		assertThat(slots.getValue(918).volatileStatusApplications.single().chancePercent).isEqualTo(100)
		...
		listOf(280L, 706L).forEach { skillId ->
			assertThat(slots.getValue(skillId).breaksTargetSideDamageReductions)
				.describedAs("击破目标侧屏障技能必须装配伤害前屏障清除: skillId=$skillId")
				.isTrue()
		}
		...
		val leechSeed = slots.getValue(73)
		assertThat(leechSeed.plantsLeechSeed).isTrue()
		assertThat(leechSeed.targetScope).isEqualTo(BattleSkillTargetScope.SELECTED_TARGET)
		assertThat(leechSeed.affectedByProtect).isTrue()

		val rapidSpin = slots.getValue(229)
		assertThat(rapidSpin.clearsUserSideHazardsAndTraps).isTrue()
		assertThat(rapidSpin.statStageEffects.single()).isEqualTo(
			BattleStatStageEffect(
				target = BattleEffectTarget.USER,
				stat = BattleStat.SPEED,
				stageDelta = 1,
		...
		val mortalSpin = slots.getValue(866)
		assertThat(mortalSpin.clearsUserSideHazardsAndTraps).isTrue()
		assertThat(mortalSpin.targetScope).isEqualTo(BattleSkillTargetScope.ALL_ADJACENT_OPPONENTS)
		val mortalSpinStatus = mortalSpin.statusApplications.single()
		assertThat(mortalSpinStatus.status).isEqualTo(BattleMajorStatus.POISON)
		assertThat(mortalSpinStatus.target).isEqualTo(BattleEffectTarget.TARGET)
		assertThat(mortalSpinStatus.chancePercent).isEqualTo(100)

		val tidyUp = slots.getValue(882)
		assertThat(tidyUp.clearsFieldHazardsAndSubstitutes).isTrue()
		assertThat(tidyUp.targetScope).isEqualTo(BattleSkillTargetScope.SELF)
		assertThat(tidyUp.affectedByProtect).isFalse()
		...
		assertThat(slots.getValue(182).protectsUser).isTrue()
		assertThat(slots.getValue(182).enduresFatalDamage).isFalse()
		assertThat(slots.getValue(197).targetScope).isEqualTo(BattleSkillTargetScope.SELF)
		assertThat(slots.getValue(197).protectsUser).isTrue()
		assertThat(slots.getValue(197).enduresFatalDamage).isFalse()
		assertThat(slots.getValue(203).targetScope).isEqualTo(BattleSkillTargetScope.SELF)
		assertThat(slots.getValue(203).protectsUser).isFalse()
		assertThat(slots.getValue(203).enduresFatalDamage).isTrue()
		assertThat(slots.getValue(364).targetScope).isEqualTo(BattleSkillTargetScope.SELECTED_TARGET)
		assertThat(slots.getValue(364).priority).isEqualTo(2)
		assertThat(slots.getValue(364).affectedByProtect).isFalse()
		assertThat(slots.getValue(364).breaksProtection).isTrue()
		assertThat(slots.getValue(469).targetScope).isEqualTo(BattleSkillTargetScope.SELF)
		assertThat(slots.getValue(469).priority).isEqualTo(3)
		assertThat(slots.getValue(469).affectedByProtect).isFalse()
		assertThat(slots.getValue(469).protectsUserSideFromMultiTargetSkills).isTrue()
		assertThat(slots.getValue(469).protectsUserSideFromPrioritySkills).isFalse()
		assertThat(slots.getValue(501).targetScope).isEqualTo(BattleSkillTargetScope.SELF)
		assertThat(slots.getValue(501).priority).isEqualTo(3)
		assertThat(slots.getValue(501).affectedByProtect).isFalse()
		assertThat(slots.getValue(501).protectsUserSideFromMultiTargetSkills).isFalse()
		assertThat(slots.getValue(501).protectsUserSideFromPrioritySkills).isTrue()
		assertThat(slots.getValue(180).targetLastSkillPpReduction).isEqualTo(4)
		assertThat(slots.getValue(336).targetScope).isEqualTo(BattleSkillTargetScope.USER_SIDE_ACTIVE)
		assertThat(slots.getValue(336).affectedByProtect).isFalse()
		assertThat(slots.getValue(336).soundBased).isTrue()
	...
	fun `ability and item rule assembly includes supported engine effects`() {
		...
		assertThat(service.abilityEffectsByAbilityId(24))
			.containsExactly(BattleAbilityEffect.ContactDamageToAttacker(damageDenominator = 8))
		assertThat(service.abilityEffectsByAbilityId(160))
			.containsExactly(BattleAbilityEffect.ContactDamageToAttacker(damageDenominator = 8))
