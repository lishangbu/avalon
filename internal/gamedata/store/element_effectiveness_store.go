...
type elementEffectivenessTransactionStore struct {
	parent   *Store
...
func (w *elementEffectivenessTransactionStore) Create(ctx context.Context, record elementeffectiveness.CreateRecord) (elementeffectiveness.Effectiveness, error) {
...
func (w *elementEffectivenessTransactionStore) Update(ctx context.Context, record elementeffectiveness.UpdateRecord) (elementeffectiveness.Effectiveness, error) {
	digest, err := idempotency.Digest(record.Effectiveness)
	if err != nil {
...
// Get 查询指定属性克制资料。
func (s *Store) GetElementEffectiveness(ctx context.Context, id snowflake.ID) (elementeffectiveness.Effectiveness, error) {
...
func (s *Store) ListElementEffectiveness(ctx context.Context, query elementeffectiveness.ListQuery) (elementeffectiveness.Page, error) {
...
func (s *Store) ListEnabledElementEffectiveness(ctx context.Context) ([]elementeffectiveness.Effectiveness, error) {
...
type ElementEffectivenessStore struct {
	// Store 是共享连接池、事务和审计实现。
	*Store
}

// NewElementEffectivenessStore 创建属性克制应用服务使用的窄存储适配器。
func NewElementEffectivenessStore(store *Store) *ElementEffectivenessStore {
	return &ElementEffectivenessStore{Store: store}
}

// Get 按稳定 Identifier 查询单条属性克制资料。
func (s *ElementEffectivenessStore) Get(ctx context.Context, id snowflake.ID) (elementeffectiveness.Effectiveness, error) {
	return s.GetElementEffectiveness(ctx, id)
}

// List 按页码和可选攻击、防御属性及启用状态筛选属性克制资料。
func (s *ElementEffectivenessStore) List(ctx context.Context, query elementeffectiveness.ListQuery) (elementeffectiveness.Page, error) {
	return s.ListElementEffectiveness(ctx, query)
}

// ListEnabled 返回 Battle 规则快照需要冻结的全部启用属性克制资料。
func (s *ElementEffectivenessStore) ListEnabled(ctx context.Context) ([]elementeffectiveness.Effectiveness, error) {
