...
type EquipmentBattleSnapshotDocumentEntry struct {
	...
	StatModifiers []EquipmentStatModifier `json:"statModifiers"`
...
func (store *EntWorldStore) FreezePlayerCharacterEquipmentForBattle(ctx context.Context, playerID snowflake.ID) (json.RawMessage, error) {
	return FreezePlayerCharacterEquipmentWithEnt(ctx, store.pool.Client(ctx), playerID)
}
...
func FreezePlayerCharacterEquipmentWithEnt(ctx context.Context, client *avalonent.Client, playerID snowflake.ID) (json.RawMessage, error) {
	if client == nil || !playerID.IsValid() {
		return nil, ErrEquipmentLoadoutConflict
	}
	...
	entries := make([]EquipmentBattleSnapshotEntry, 0, len(rows))
	for _, row := range rows {
		...
		modifiers := make([]EquipmentStatModifier, 0, len(equipment.Edges.StatModifiers))
		...
		entries = append(entries, EquipmentBattleSnapshotEntry{InstanceID: instance.ID, EquipmentID: equipment.ID, ItemID: equipment.ItemID, Slot: EquipmentSlot(row.Slot), CatalogVersion: equipment.Version, StatModifiers: modifiers, CompiledRules: equipment.Rules})
	}
	snapshot, err := FreezeEquipmentBattleSnapshot(loadoutVersion, entries)
	if err != nil {
		return nil, err
	}
