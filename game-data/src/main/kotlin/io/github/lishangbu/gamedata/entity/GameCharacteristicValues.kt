 ...
 * 个体特征取值持久化实体。
 *
 * 对应 `game_characteristic_value` 表，仅承载一条个体特征取值记录及其标量属性。
 * 关联项保留为独立 ID，避免把其他功能的实体聚合进当前资料对象；新增记录的主键统一由 CosId 生成。
 */
@Entity
@Table(name = "game_characteristic_value")
interface GameCharacteristicValues {
	/**
	 * 个体特征取值记录的唯一标识。
	 *
	 ...
	 * 个体特征取值的可能取值。
	 *
	 * 对应数据库 `possible_value` 列，写入时必须提供。
	 */
