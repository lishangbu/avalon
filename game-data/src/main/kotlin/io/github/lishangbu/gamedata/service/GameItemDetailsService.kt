...
import io.github.lishangbu.gamedata.entity.flingEffectId
...
class GameItemDetailsService(
	...
	): Page<GameItemDetailsResponse> {
		validatePage(page, size)
		val search = searchFilter(query)
		return sqlClient.createQuery(GameItemDetails::class) {
			search.pattern?.let { pattern ->
				where(or(sql<String>("cast(%e as text)", table.itemId) ilike pattern, table.effect ilike pattern, table.flavorText ilike pattern))
			}
			filters.forEach { (field, rawValue) ->
				when (field) {
				"item_id" -> gameDataLongFilterValue("item_id", rawValue)?.let { where(table.itemId eq it) }
				"fling_effect_id" -> gameDataLongFilterValue("fling_effect_id", rawValue)?.let { where(table.flingEffectId eq it) }
	...
	fun create(request: GameItemDetailsRequest): GameItemDetailsResponse =
		repository.save(
			GameItemDetails {
				...
				flingEffectId = request.flingEffectId
	...
	private fun entityByIdOrNotFound(id: Long): GameItemDetails =
		repository.findNullable(id) ?: notFound("id", "道具详情不存在: $id")

	private fun GameItemDetails.toResponse(): GameItemDetailsResponse =
		GameItemDetailsResponse {
			id = this@toResponse.id
			itemId = this@toResponse.itemId
			flingEffectId = this@toResponse.flingEffectId
