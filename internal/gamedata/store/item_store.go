...
// itemTransactionStore 闅旂閬撳叿璧勬枡 Writer 鐨勬柟娉曢泦鍚堛€?
type itemTransactionStore struct {
	parent   *Store
...
func (w *itemTransactionStore) Create(ctx context.Context, record item.CreateRecord) (item.Item, error) {
...
func (s *Store) GetItem(ctx context.Context, itemID snowflake.ID) (item.Item, error) {
...
func (s *Store) ListItems(ctx context.Context, query item.ListQuery) (item.Page, error) {
...
func (w *itemTransactionStore) Update(ctx context.Context, record item.UpdateRecord) (item.Item, error) {
	digest, err := idempotency.Digest(struct {
		ItemID          snowflake.ID
...
func (w *itemTransactionStore) Disable(ctx context.Context, record item.DisableRecord) error {
	digest, err := idempotency.Digest(struct {
		ItemID          snowflake.ID
		ExpectedVersion int64
