...
// NewElementEffectivenessRepository 创建属性克制应用服务使用的窄关系型持久化适配器。
func NewElementEffectivenessRepository(adapters *Adapters) *ElementEffectivenessRepository {
	return &ElementEffectivenessRepository{Adapters: adapters}
}

// Get 按稳定 Identifier 查询单条属性克制资料。
func (s *ElementEffectivenessRepository) Get(ctx context.Context, id snowflake.ID) (elementeffectiveness.Effectiveness, error) {
	return s.GetElementEffectiveness(ctx, id)
}

// List 按页码和可选攻击、防御属性及启用状态筛选属性克制资料。
func (s *ElementEffectivenessRepository) List(ctx context.Context, query elementeffectiveness.ListQuery) (elementeffectiveness.Page, error) {
	return s.ListElementEffectiveness(ctx, query)
}

// ListEnabled 返回 Battle 规则快照需要冻结的全部启用属性克制资料。
func (s *ElementEffectivenessRepository) ListEnabled(ctx context.Context) ([]elementeffectiveness.Effectiveness, error) {
