 ...
 * 读取必填文本字段，并限制最大长度。
 */
internal fun String.requiredText(fieldName: String, maxLength: Int, minLength: Int = 1): String {
	val value = trim()
	if (value.isBlank()) {
 ...
 * 读取稳定 slug/code 字段。
 */
internal fun String.requiredSlugCode(fieldName: String): String {
 ...
 * 读取用户名字段。
 */
internal fun String.requiredUsername(fieldName: String): String {
	val value = requiredText(fieldName, maxLength = 64)
 ...
 * 读取密码字段，并限制长度。
 */
internal fun String.requiredPassword(fieldName: String): String {
	val value = trim()
	if (value.isBlank()) {
		throw requiredField(fieldName)
	}
 ...
 * 规范化访问节点 code 列表。
 */
internal fun List<String>.normalizedAccessNodeCodes(fieldName: String): List<String> {
 ...
 * 规范化 slug/code 列表。
 */
internal fun List<String>.normalizedSlugCodes(fieldName: String): List<String> {
	val values = normalizedList(fieldName)
 ...
 * 确保字段值属于允许集合。
 */
internal fun String.requiredSupportedValue(fieldName: String, supportedValues: Set<String>): String {
	val value = requiredText(fieldName, maxLength = 64)
 ...
 * 确保列表全部属于允许集合。
 */
internal fun List<String>.requireSupportedValues(fieldName: String, supportedValues: Set<String>) {
 ...
 * 校验数值字段处于闭区间范围内。
 */
internal fun Long.requiredRange(fieldName: String, min: Long, max: Long): Long {
	if (this !in min..max) {
 ...
 * 抛出字段非法错误。
 */
internal fun invalidValue(fieldName: String, message: String): Nothing =
	throw invalidField(fieldName, message)

/**
 * 抛出资源冲突错误。
 */
internal fun conflict(fieldName: String, message: String): Nothing =
 ...
 * 抛出关联资源不存在或不可用错误。
 */
internal fun invalidReference(fieldName: String, message: String): Nothing =
	throw invalidField(fieldName, message)

/**
 * 抛出资源不存在错误。
 */
internal fun notFound(fieldName: String, message: String): Nothing =
	throw SystemApiException(
...
private fun requiredField(fieldName: String): SystemApiException =
	SystemApiException(
		status = HttpStatus.BAD_REQUEST,
		code = SystemApiErrorCode.VALIDATION_REQUIRED,
		message = "$fieldName 不能为空",
		field = fieldName,
	)

private fun invalidField(fieldName: String, message: String): SystemApiException =
	SystemApiException(
		status = HttpStatus.BAD_REQUEST,
...
private fun unsupportedField(fieldName: String, message: String): SystemApiException =
	SystemApiException(
		status = HttpStatus.BAD_REQUEST,
		code = SystemApiErrorCode.VALIDATION_UNSUPPORTED,
		message = message,
		field = fieldName,
	)
