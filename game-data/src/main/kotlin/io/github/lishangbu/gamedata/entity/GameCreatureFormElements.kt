 ...
 * 精灵形态属性持久化实体。
 *
 * 对应 `game_creature_form_element` 表，仅承载一条精灵形态属性记录及其标量属性。
 * 关联项保留为独立 ID，避免把其他功能的实体聚合进当前资料对象；新增记录的主键统一由 CosId 生成。
 */
@Entity
@Table(name = "game_creature_form_element")
interface GameCreatureFormElements {
	/**
	 * 精灵形态属性记录的唯一标识。
	 *
	 ...
	 * 精灵形态属性引用的形态 ID。
	 *
	 * 对应数据库 `form_id` 列，写入时必须提供；关联标识以 Long 标量保存，不在该实体中聚合关联对象。
	 */
	val formId: Long
