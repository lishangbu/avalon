 ...
 * 进化链持久化实体。
 *
 * 对应 `game_evolution_chain` 表，仅承载一条进化链记录及其标量属性。
 * 关联项保留为独立 ID，避免把其他功能的实体聚合进当前资料对象；新增记录的主键统一由 CosId 生成。
 */
@Entity
@Table(name = "game_evolution_chain")
interface GameEvolutionChains {
	/**
	 * 进化链记录的唯一标识。
	 *
	 ...
	 * 进化链引用的幼体触发道具 ID。
	 *
	 * 对应数据库 `baby_trigger_item_id` 列，允许为空；关联标识以 Long 标量保存，不在该实体中聚合关联对象。
	 */
