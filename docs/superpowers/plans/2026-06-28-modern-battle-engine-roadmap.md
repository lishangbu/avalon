...
- 资料数据内容可以保留实际条目文本，例如现有精灵、技能、特性、道具的中文名称。
...
| 游戏资料 | 支撑域 | 继续提供精灵、种类、技能、特性、道具、属性、数值项等静态资料。 |
| 管理端 | 支撑域 | 维护规则资料、对战格式和规则覆盖说明。 |
...
- 已存在。
- 保存静态资料，不参与战斗过程计算。
- 输出给战斗规则模块的内容是稳定引用对象：技能、特性、道具、属性、数值项、精灵和种类。
- 不新增版本维度。
...
- 页面用于规则资料 CRUD 和规则预览；公开规则对照场景只由单元测试源码维护。

## 统一术语

| 术语 | 含义 |
| --- | --- |
| 精灵 `creature` | 可参与战斗的具体资料条目，对应现有 `game_creature`。 |
| 种类 `species` | 精灵所属的种类资料，对应现有 `game_species`。 |
| 技能 `skill` | 战斗中可选择的行动能力，对应现有 `game_skill`。 |
| 特性 `ability` | 精灵被动能力，对应现有 `game_ability`。 |
...
| 公开规则场景 | 用于验证规则的固定输入、随机序列、期望事件和期望状态，事实源是单元测试源码。 |
...
- 每个公开规则场景要说明规则场景、参考来源类型、固定随机序列意图和验证重点。
...
- 每个复杂规则都需要至少一个公开规则场景。
- 公开规则场景不保存外部项目代码，只保存输入、行动、随机序列、期望事件和期望最终状态。
- 公开规则场景不提供管理端 CRUD；新增或修改规则时直接维护 Kotlin 测试，避免场景表和代码场景两套事实源。

### Kotlin 公开规则场景约定

- 用 `assertNamed("规则场景名")` 注册稳定场景锚点，覆盖账本按场景名生成规则编号映射。
- 用测试工厂直接构造 `BattleInitialState`、规则快照、行动序列和必要的运行态。
- 随机行为统一使用 `ScriptedBattleRandom` 或同等固定随机源，不依赖真实随机数。
- 断言必须覆盖关键 `BattleEvent` 顺序和最终 `BattleState`，复杂规则还要断言边界状态没有被意外改写。
...
7. `test(battle-engine)`: 引入第一批公开对照规则场景。
...
- 单打 MVP 通过公开对照用例。
- 双打 MVP 通过公开对照用例。
- 管理端能维护规则资料；规则覆盖由 Kotlin 测试账本验证。
