 ...
 *
 * 场景类型：伤害写入前的保命特性和一次性保命携带道具 场景。
 * 参考来源类型：公开成熟模拟器实现和公开规则说明。该规则只在技能直接伤害会从满 HP 打倒目标时触发；
 * 特性来源不消耗，道具来源触发后移除携带道具。非满 HP、间接伤害和后续绕过特性的规则将由各自 场景 扩展。
 */
class BattleFatalDamageSurvivalTests {
	...
	fun `full hp survival ability leaves target at one hp before faint`() {
		...
		scenario.assertNamed("full-hp-survival-ability-leaves-one-hp-before-faint")
	...
	fun `consumable full hp survival item leaves target at one hp and consumes item`() {
		...
		scenario.assertNamed("consumable-full-hp-survival-item-leaves-one-hp-and-consumes-item")
	...
	fun `full hp survival does not trigger after prior damage`() {
		...
		scenario.assertNamed("full-hp-survival-does-not-trigger-after-prior-damage")
