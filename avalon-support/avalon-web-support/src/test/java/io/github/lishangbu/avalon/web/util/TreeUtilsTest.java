...
class TreeUtilsTest {
    ...
    void getNodePath_shouldReturnEmptyListWhenNotFound() {
        ...
        List<TreeNode> result =
                TreeUtils.getNodePath(
                        treeNodes,
                        node -> Objects.equals(node.getId(), 999L),
                        TreeNode::getChildren);
    ...
    void flattenTree_shouldHandleEmptyTree() {
        // 扁平化空树
        List<TreeNode> result =
                TreeUtils.flattenTree(Collections.emptyList(), TreeNode::getChildren);
    ...
    void findNodeById_shouldReturnCorrectNode() {
        // 根据ID查找节点
        TreeNode result =
                TreeUtils.findNodeById(treeNodes, 6L, TreeNode::getId, TreeNode::getChildren);
