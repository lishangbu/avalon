...
func (s *Adapters) ListItemDictionary(ctx context.Context, kind itemdictionary.Kind) ([]itemdictionary.Entry, error) {
...
func (s *Adapters) CreateItemDictionary(ctx context.Context, entry itemdictionary.Entry, write administration.GameDataWriteContext, createdAt time.Time) (itemdictionary.Entry, error) {
	return s.writeItemDictionary(ctx, entry, 0, write, createdAt, true)
}

// UpdateItemDictionary 在单个事务中使用乐观版本更新字典、审计与幂等响应。
func (s *Adapters) UpdateItemDictionary(ctx context.Context, entry itemdictionary.Entry, expectedVersion int64, write administration.GameDataWriteContext, updatedAt time.Time) (itemdictionary.Entry, error) {
	return s.writeItemDictionary(ctx, entry, expectedVersion, write, updatedAt, false)
}

func (s *Adapters) writeItemDictionary(ctx context.Context, entry itemdictionary.Entry, expectedVersion int64, write administration.GameDataWriteContext, at time.Time, create bool) (itemdictionary.Entry, error) {
