...
import io.github.lishangbu.gamedata.entity.formula
...
class GameGrowthRatesService(
	...
	): Page<GameGrowthRatesResponse> {
		validatePage(page, size)
		val search = searchFilter(query)
		return sqlClient.createQuery(GameGrowthRates::class) {
			...
			filters.forEach { (field, rawValue) ->
				...
				"formula" -> gameDataStringFilterValue("formula", rawValue)?.let { where(table.formula eq it) }
	...
	fun create(request: GameGrowthRatesRequest): GameGrowthRatesResponse =
		repository.save(
			GameGrowthRates {
				...
				formula = gameDataOptionalText(request.formula, "formula", null)
	...
	private fun entityByIdOrNotFound(id: Long): GameGrowthRates =
		repository.findNullable(id) ?: notFound("id", "成长速率不存在: $id")

	private fun GameGrowthRates.toResponse(): GameGrowthRatesResponse =
		GameGrowthRatesResponse {
			id = this@toResponse.id
			code = this@toResponse.code
			name = this@toResponse.name
			formula = this@toResponse.formula
