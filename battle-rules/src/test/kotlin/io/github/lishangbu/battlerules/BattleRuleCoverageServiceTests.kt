...
class BattleRuleCoverageServiceTests {
	...
	fun `coverage summary matches item statuses`() {
		...
		assertEquals(240, coverage.targetSummary.coveredRuleCount)
		assertEquals(72, coverage.targetSummary.remainingRuleCount)
		assertEquals(76, coverage.targetSummary.implementationPercent)
		...
		assertTrue(coverage.items.any { it.code == "damage.formula-boundaries" && it.status == "IMPLEMENTED" })
		...
		assertTrue(coverage.items.any { it.code == "damage.proportional-damage" && it.status == "IMPLEMENTED" })
		assertTrue(coverage.items.any { it.code == "damage.hp-derived-damage" && it.status == "IMPLEMENTED" })
		assertTrue(coverage.items.any { it.code == "damage.critical-hit-flow" && it.status == "IMPLEMENTED" })
		...
		assertTrue(coverage.items.any { it.code == "status.heal-block" && it.status == "IMPLEMENTED" })
		assertTrue(coverage.items.any { it.code == "status.taunt" && it.status == "IMPLEMENTED" })
		...
		assertTrue(coverage.items.any { it.code == "hit.defense-boundaries" && it.status == "IMPLEMENTED" })
		assertTrue(coverage.items.any { it.code == "turn.action-ordering" && it.status == "IMPLEMENTED" })
		assertTrue(coverage.items.any { it.code == "lifecycle.switch-faint-result" && it.status == "IMPLEMENTED" })
		assertTrue(coverage.items.any { it.code == "field.environment-boundaries" && it.status == "IMPLEMENTED" })
	}
}
