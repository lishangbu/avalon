...
// managementAdapters 持久化管理员账号维护并读取安全审计。
type managementAdapters struct {
	pool      *database.Pool
	ids       snowflake.Source
	passwords *securityaccount.PasswordHasher
}

// NewManagementAdapters 创建管理员维护持久化适配器。
func NewManagementAdapters(pool *database.Pool, ids snowflake.Source) *managementAdapters {
	return &managementAdapters{pool: pool, ids: ids, passwords: securityaccount.NewPasswordHasher(rand.Reader)}
}

// ListAccounts 返回不包含密码资料的管理员账号。
func (s *managementAdapters) ListAccounts(ctx context.Context, pageSize int) ([]admin.ManagedAccount, error) {
	if pageSize < 1 {
		pageSize = 50
	}
	if pageSize > 200 {
...
func (s *managementAdapters) CreateAccount(ctx context.Context, command admin.CreateManagedAccountCommand) (admin.ManagedAccount, error) {
...
func (s *managementAdapters) SetAccountEnabled(ctx context.Context, command admin.SetManagedAccountEnabledCommand) (admin.ManagedAccount, error) {
...
func (s *managementAdapters) ListAuditLogs(ctx context.Context, pageSize int) ([]admin.AuditLog, error) {
	if pageSize < 1 {
		pageSize = 50
	}
	if pageSize > 200 {
