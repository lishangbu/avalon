...
class BattleHpDerivedDamageSkillTests {
	...
	fun `HP difference damage reduces target to user current hp`() {
		...
		scenario.assertNamed("hp-difference-damage-skill-reduces-target-to-user-current-hp")
	...
	fun `HP difference damage fails when target hp is not greater than user hp`() {
		...
		scenario.assertNamed("hp-difference-damage-skill-fails-when-target-not-above-user")
	...
	fun `user current hp sacrifice damage faints user after hit`() {
		...
		scenario.assertNamed("user-current-hp-sacrifice-damage-skill-faints-user")
