...
func (store *EntWorldStore) ListEquipmentInstances(ctx context.Context, accountID snowflake.ID, size int, cursor string) (EquipmentInstancePage, error) {
	client := store.pool.Client(ctx)
	playerID, err := activePlayerCharacterID(ctx, client, accountID)
	if err != nil {
		return EquipmentInstancePage{}, err
	}
	if size < 1 || size > 100 {
		size = 50
	}
	...
	if err != nil {
		return EquipmentInstancePage{}, fmt.Errorf("查询 Equipment Instance: %w", err)
	}
	hasMore := len(rows) > size
	if hasMore {
		rows = rows[:size]
	}
	result := EquipmentInstancePage{Items: make([]EquipmentInstance, 0, len(rows))}
	for _, row := range rows {
		value, mapErr := equipmentInstanceView(row)
		if mapErr != nil {
			return EquipmentInstancePage{}, mapErr
		}
...
func (store *EntWorldStore) GetEquipmentInstance(ctx context.Context, accountID, instanceID snowflake.ID) (EquipmentInstance, error) {
	client := store.pool.Client(ctx)
	playerID, err := activePlayerCharacterID(ctx, client, accountID)
	if err != nil {
		return EquipmentInstance{}, err
	}
	...
	if avalonent.IsNotFound(err) {
		return EquipmentInstance{}, ErrEquipmentNotOwned
	}
	if err != nil {
		return EquipmentInstance{}, err
	}
...
func (store *EntWorldStore) GetEquipmentLoadout(ctx context.Context, accountID snowflake.ID) (EquipmentLoadout, error) {
	client := store.pool.Client(ctx)
	playerID, err := activePlayerCharacterID(ctx, client, accountID)
	if err != nil {
...
func (store *EntWorldStore) ReplaceEquipmentLoadout(ctx context.Context, command ReplaceEquipmentLoadoutCommand) (EquipmentLoadout, error) {
	var result EquipmentLoadout
	...
	err := store.pool.WithinTransaction(ctx, func(txctx context.Context) error {
		...
		replayed, err := store.claimPlayerResponse(txctx, client, playerID, "rpg.equipment.loadout.replace", command.IdempotencyKey, digest[:], &result, command.Now.UTC())
		if err != nil || replayed {
			return err
		}
		...
		candidates := make([]EquipmentLoadoutCandidate, 0, len(command.Entries))
		for _, entry := range command.Entries {
			row := byID[entry.InstanceID]
			if row == nil || row.Edges.Equipment == nil {
				return ErrEquipmentNotOwned
			}
			...
			candidates = append(candidates, EquipmentLoadoutCandidate{Slot: entry.Slot, InstanceID: entry.InstanceID, SlotType: EquipmentSlotType(equipment.SlotType), Handedness: EquipmentHandedness(optionalString(equipment.Handedness)), MinimumLevel: equipment.MinimumLevel, ProfessionIDs: allowed})
		}
		if err := ValidateEquipmentLoadout(character.Level, professions, candidates); err != nil {
			return err
		}
		...
		for _, old := range oldRows {
			if newBySlot[old.Slot] != old.EquipmentInstanceID {
				if err := store.createEquipmentTransaction(txctx, client, operationID, playerID, old.EquipmentInstanceID, "unequip", "", old.Slot, command.Now.UTC()); err != nil {
					return err
				}
			}
		}
		...
		for _, entry := range command.Entries {
			...
			if err := store.createEquipmentTransaction(txctx, client, operationID, playerID, entry.InstanceID, "equip", "", string(entry.Slot), command.Now.UTC()); err != nil {
				return err
			}
		}
		...
		result = EquipmentLoadout{Version: state.Version, Entries: append([]EquipmentLoadoutEntry(nil), command.Entries...), UpdatedAt: state.UpdatedAt}
		if err := store.createEquipmentOutbox(txctx, client, "rpg.equipment.loadout-replaced.v1", operationID, playerID, command.Now.UTC()); err != nil {
			return err
		}
		return store.completePlayerResponse(txctx, client, playerID, "rpg.equipment.loadout.replace", command.IdempotencyKey, result)
	})
	return result, err
}

// SellEquipmentInstance 原子写入未穿戴实例出售终态、钱包余额、资产流水、幂等响应与 Outbox。
func (store *EntWorldStore) SellEquipmentInstance(ctx context.Context, command SellEquipmentCommand) (SellEquipmentResult, error) {
	var result SellEquipmentResult
	...
	err := store.pool.WithinTransaction(ctx, func(txctx context.Context) error {
		...
		digest := sha256.Sum256(payload)
		replayed, err := store.claimPlayerResponse(txctx, client, playerID, "rpg.equipment.sell", command.IdempotencyKey, digest[:], &result, command.Now.UTC())
		if err != nil || replayed {
			return err
		}
		...
		if err = store.createEquipmentTransaction(txctx, client, operationID, playerID, row.ID, "sell", "", "", command.Now.UTC()); err != nil {
			return err
		}
		equipment := row.Edges.Equipment
		result = SellEquipmentResult{OperationID: operationID, CurrencyID: equipment.SellCurrencyID, SellPrice: equipment.SellPrice}
		balanceAfter, err := store.creditEquipmentSale(txctx, client, playerID, equipment.SellCurrencyID, equipment.SellPrice, operationID, command.Now.UTC())
		if err != nil {
			return err
		}
		...
		if err = store.createEquipmentOutbox(txctx, client, "rpg.equipment.sold.v1", operationID, playerID, command.Now.UTC()); err != nil {
			return err
		}
		return store.completePlayerResponse(txctx, client, playerID, "rpg.equipment.sell", command.IdempotencyKey, result)
	})
	return result, err
}
...
func (store *EntWorldStore) creditEquipmentSale(ctx context.Context, client *avalonent.Client, playerID, currencyID snowflake.ID, amount int64, operationID snowflake.ID, now time.Time) (int64, error) {
	...
	if avalonent.IsNotFound(err) {
		walletID, idErr := store.newID.Next(ctx)
		if idErr != nil {
			return 0, idErr
		}
...
func readEquipmentLoadout(ctx context.Context, client *avalonent.Client, playerID snowflake.ID) (EquipmentLoadout, error) {
	...
	if avalonent.IsNotFound(err) {
		return EquipmentLoadout{Version: 1, Entries: []EquipmentLoadoutEntry{}, UpdatedAt: time.Unix(0, 0).UTC()}, nil
	}
	if err != nil {
		return EquipmentLoadout{}, err
	}
	...
	result := EquipmentLoadout{Version: state.Version, Entries: make([]EquipmentLoadoutEntry, 0, len(rows)), UpdatedAt: state.UpdatedAt}
	for _, row := range rows {
		result.Entries = append(result.Entries, EquipmentLoadoutEntry{Slot: EquipmentSlot(row.Slot), InstanceID: row.EquipmentInstanceID, EquippedAt: row.EquippedAt})
	}
	return result, nil
}

func equipmentInstanceView(row *avalonent.PlayerCharacterEquipmentInstance) (EquipmentInstance, error) {
	equipment := row.Edges.Equipment
	if equipment == nil || equipment.Edges.Item == nil {
		return EquipmentInstance{}, fmt.Errorf("Equipment Instance %s 缺少资料关系", row.ID)
	}
	value := EquipmentInstance{ID: row.ID, EquipmentID: row.EquipmentID, ItemID: equipment.ItemID, Name: equipment.Edges.Item.Name, SlotType: EquipmentSlotType(equipment.SlotType), Handedness: EquipmentHandedness(optionalString(equipment.Handedness)), SourceType: row.SourceType, Version: row.Version, MinimumLevel: equipment.MinimumLevel, RuleTimings: equipmentRuleTimings(equipment.Rules), AcquiredAt: row.AcquiredAt}
...
func (store *EntWorldStore) createEquipmentTransaction(ctx context.Context, client *avalonent.Client, operationID, playerID, instanceID snowflake.ID, action, source, slot string, now time.Time) error {
...
func (store *EntWorldStore) createEquipmentOutbox(ctx context.Context, client *avalonent.Client, topic string, operationID, playerID snowflake.ID, now time.Time) error {
	id, err := store.newID.Next(ctx)
	if err != nil {
		return err
	}
