...
func TestLifecycleServiceExpiresEveryDueLifecycleKind(t *testing.T) {
	...
	repository := &lifecycleAdaptersStub{
...
type lifecycleAdaptersStub struct {
...
func (repository *lifecycleAdaptersStub) ListExpiredChallengeIDs(context.Context, time.Time) ([]snowflake.ID, error) {
	return append([]snowflake.ID(nil), repository.expiredChallenges...), nil
}

func (repository *lifecycleAdaptersStub) ExpireChallenge(_ context.Context, _ snowflake.ID, observedAt time.Time) (battle.Challenge, error) {
	repository.calledAt = append(repository.calledAt, observedAt)
	return battle.Challenge{}, nil
}

func (repository *lifecycleAdaptersStub) ListExpiredPreviewBattleIDs(context.Context, time.Time) ([]snowflake.ID, error) {
	return append([]snowflake.ID(nil), repository.expiredPreviews...), nil
}

func (repository *lifecycleAdaptersStub) CompleteExpiredPreview(_ context.Context, _ snowflake.ID, observedAt time.Time) (battle.Battle, error) {
	repository.calledAt = append(repository.calledAt, observedAt)
	return battle.Battle{}, nil
}

func (repository *lifecycleAdaptersStub) ListExpiredRunningBattleIDs(context.Context, time.Time) ([]snowflake.ID, error) {
	return append([]snowflake.ID(nil), repository.expiredBattlees...), nil
}

func (repository *lifecycleAdaptersStub) CompleteBattleTimeout(_ context.Context, _ snowflake.ID, observedAt time.Time) (battle.Battle, error) {
	repository.calledAt = append(repository.calledAt, observedAt)
	return battle.Battle{}, nil
}

func (repository *lifecycleAdaptersStub) ScheduleMissingRuntimeRecoveries(_ context.Context, observedAt time.Time, maximum int) (int, error) {
	repository.calledAt = append(repository.calledAt, observedAt)
	if maximum != 100 {
...
func (repository *lifecycleAdaptersStub) allCallsAt(expected time.Time) bool {
