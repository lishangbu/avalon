 ...
 * 地点索引持久化实体。
 *
 * 对应 `game_location_game_index` 表，仅承载一条地点索引记录及其标量属性。
 * 关联项保留为独立 ID，避免把其他功能的实体聚合进当前资料对象；新增记录的主键统一由 CosId 生成。
 */
@Entity
@Table(name = "game_location_game_index")
interface GameLocationGameIndices {
	/**
	 * 地点索引记录的唯一标识。
	 *
	 ...
	 * 地点索引引用的地点 ID。
	 *
	 * 对应数据库 `location_id` 列，写入时必须提供；关联标识以 Long 标量保存，不在该实体中聚合关联对象。
	 */
	val locationId: Long
