...
class BattleDamageFormulaBoundaryPublicReferenceTests {
	...
	fun `level fifty neutral same element physical damage keeps base formula`() {
		...
		scenario.assertNamed("level-fifty-neutral-same-element-physical-damage-keeps-base-formula")
	...
	fun `non matching element skips same element bonus`() {
		...
		scenario.assertNamed("non-matching-element-skips-same-element-bonus")
	...
	fun `dual type effectiveness multiplies every defending element`() {
		...
		scenario.assertNamed("dual-type-effectiveness-multiplies-every-defending-element")
	...
	fun `type immunity keeps final damage at zero without minimum damage`() {
		...
		scenario.assertNamed("type-immunity-keeps-final-damage-at-zero-without-minimum-damage")
	...
	fun `positive non immune damage is floored to at least one`() {
		...
		scenario.assertNamed("positive-non-immune-damage-is-floored-to-at-least-one")
	...
	fun `damage random percent is applied after base damage`() {
		...
		scenario.assertNamed("damage-random-percent-is-applied-after-base-damage")
		assertEquals(19, result.baseDamage)
	...
	fun `spread target modifier participates in final multiplier chain`() {
		...
		scenario.assertNamed("spread-target-modifier-participates-in-final-multiplier-chain")
	...
	fun `side damage reduction multiplier participates in final multiplier chain`() {
		...
		scenario.assertNamed("side-damage-reduction-multiplier-participates-in-final-multiplier-chain")
	...
	fun `critical hit applies one and half final damage multiplier`() {
		...
		scenario.assertNamed("critical-hit-applies-one-and-half-final-damage-multiplier")
	...
	fun `burn halves physical attack before base damage`() {
		...
		scenario.assertNamed("burn-halves-physical-attack-before-base-damage")
	...
	fun `positive attack stage increases physical base damage before final multipliers`() {
		...
		scenario.assertNamed("positive-attack-stage-increases-physical-base-damage-before-final-multipliers")
	...
	fun `positive defense stage reduces physical base damage before final multipliers`() {
		...
		scenario.assertNamed("positive-defense-stage-reduces-physical-base-damage-before-final-multipliers")
		assertEquals(10, result.baseDamage)
		assertEquals(15, result.amount)
	}
	...
	fun `critical hit ignores unfavorable attack and favorable defense stages`() {
		...
		scenario.assertNamed("critical-hit-ignores-unfavorable-attack-and-favorable-defense-stages-in-formula")
		assertEquals(19, result.baseDamage)
		assertEquals(42, result.amount)
	}
	...
	fun `critical hit keeps burn physical attack penalty`() {
		...
		scenario.assertNamed("critical-hit-keeps-burn-physical-attack-penalty")
		assertEquals(10, result.baseDamage)
	...
	fun `special damage uses special attack and special defense stats`() {
		...
		scenario.assertNamed("special-damage-uses-special-attack-and-special-defense-stats")
		assertEquals(37, result.baseDamage)
		assertEquals(55, result.amount)
	}
	...
	fun `sun boosts fire damage in final multiplier chain`() {
		...
		scenario.assertNamed("sun-boosts-fire-damage-in-final-multiplier-chain")
	...
	fun `rain weakens fire damage in final multiplier chain`() {
		...
		scenario.assertNamed("rain-weakens-fire-damage-in-final-multiplier-chain")
	...
	fun `grassy terrain boosts grounded grass damage`() {
		...
		scenario.assertNamed("grassy-terrain-boosts-grounded-grass-damage-in-formula")
	...
	fun `grassy terrain weakens tagged ground move against grounded target`() {
		...
		scenario.assertNamed("grassy-terrain-weakens-tagged-ground-move-against-grounded-target")
	...
	fun `power stage item changes base damage before final multipliers`() {
		...
		scenario.assertNamed("power-stage-item-changes-base-damage-before-final-multipliers")
		assertEquals(21, result.baseDamage)
		assertEquals(31, result.amount)
	}

	private fun damageScenario(
		name: String,
		inputSummary: String,
