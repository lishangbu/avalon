...
// statTransactionRepository 在调用方开启的 Ent 事务中实现数值项资料写入端口。
type statTransactionRepository struct {
	parent   *Adapters
...
func (w *statTransactionRepository) Create(ctx context.Context, record stat.CreateRecord) (stat.Stat, error) {
...
func (s *Adapters) GetStat(ctx context.Context, statID snowflake.ID) (stat.Stat, error) {
...
func (s *Adapters) ListStats(ctx context.Context, query stat.ListQuery) (stat.Page, error) {
...
func (w *statTransactionRepository) Update(ctx context.Context, record stat.UpdateRecord) (stat.Stat, error) {
	digest, err := idempotency.Digest(struct {
		StatID          snowflake.ID
...
func (w *statTransactionRepository) Disable(ctx context.Context, record stat.DisableRecord) error {
	digest, err := idempotency.Digest(struct {
		StatID          snowflake.ID
		ExpectedVersion int64
