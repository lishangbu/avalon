...
class IpToLocationSearcherTest {
    @Autowired
    private lateinit var ipToLocationSearcher: IpToLocationSearcher

    @Test
    fun testIpQuery() {
        val ipResult = ipToLocationSearcher.ipQuery("39.189.23.43")
        requireNotNull(ipResult)
    ...
    @Test
    fun testIpQueryWithEmptyIp() {
        assertThrows(EmptyIpAddressException::class.java) { ipToLocationSearcher.ipQuery("") }
    }
}
