 ...
 * 资源 ID 稳定，便于公开 场景、管理端展示和回放记录长期对齐。
 */
class BattleValidationPublicReferenceTests {
	...
	fun `level cap rejects over level participant`() {
		...
		scenario.assertNamed("level-cap-rejects-over-level-participant")
	...
	fun `banned creature rule rejects restricted creature`() {
		...
		scenario.assertNamed("banned-creature-rule-rejects-restricted-creature")
	...
	fun `banned skill rule rejects restricted skill`() {
		...
		scenario.assertNamed("banned-skill-rule-rejects-restricted-skill")
	...
	fun `banned ability rule rejects restricted ability`() {
		...
		scenario.assertNamed("banned-ability-rule-rejects-restricted-ability")
	...
	fun `unique creature clause rejects duplicates only inside one side`() {
		...
		scenario.assertNamed("unique-creature-clause-rejects-duplicates-only-inside-one-side")
	...
	fun `unique item clause rejects duplicate held items only inside one side`() {
		...
		scenario.assertNamed("unique-item-clause-rejects-duplicate-held-items-only-inside-one-side")
	...
	fun `duplicate action submission reports every duplicated actor action`() {
		...
		scenario.assertNamed("duplicate-action-submission-reports-every-duplicated-actor-action")
	...
	fun `skill with no pp is rejected before engine resolution`() {
		...
		scenario.assertNamed("skill-with-no-pp-is-rejected-before-engine-resolution")
	...
	fun `choice locked participant cannot submit a different skill`() {
		...
		scenario.assertNamed("choice-locked-participant-cannot-submit-a-different-skill")
