		...
		val correctedMachineTerms = queryMaps(
			...
			blocked_text_patterns(pattern) as (
				values
					...
					('%本回合中移动的一对%'),
					('%使用过回合%'),
					-- 对战说明里的 turn、field 和 ally 旧数据曾被直译成“转弯”“字段”“友好生物”；
					-- 这些词会削弱规则文本的可读性，因此在导入测试中固定拦截并统一改成“回合”“场地”“友方生物”。
					('%转弯%'),
					('%该字段%'),
					('%进入该字段%'),
					('%离开该字段%'),
					...
					('%友好生物%'),
					('%友好的草生物%'),
					-- substitute 和 replacement 在不同规则里分别表示“替身”和“换入生物”；
					...
					-- “战场”“敌对生物”“替换生物”容易像直译说明书，因此在种子数据层直接拦截。
					('%进入战场%'),
					('%自进入战场%'),
					('%战场上存在%'),
					('%敌对生物%'),
					...
					('%有意识的队伍生物%'),
					('%被送出%'),
					-- 野外效果里的 lead party member 统一翻成“同行队伍首位的生物”，避免“主力/领头/排名第一”混用。
					('%主力生物%'),
					('%领头的生物%'),
					('%队伍中的第一只生物%'),
					('%队伍中排名第一%'),
